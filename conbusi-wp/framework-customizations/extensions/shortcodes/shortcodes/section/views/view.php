...
if ( ! empty( $atts['padding_top'] ) ) {
	$top_pad = 'padding-top:' . $atts['padding_top'] . ';';
}

$right_pad = '';
if ( ! empty( $atts['padding_right'] ) ) {
	$right_pad = 'padding-right:' . $atts['padding_right'] . ';';
}

$bottom_pad = '';
if ( ! empty( $atts['padding_bottom'] ) ) {
	$bottom_pad = 'padding-bottom:' . $atts['padding_bottom'] . ';';
}

$left_pad = '';
if ( ! empty( $atts['padding_left'] ) ) {
	$left_pad = 'padding-left:' . $atts['padding_left'] . ';';
}
...
$section_style   = ( $bg_color || $bg_image ) ? 'style="' . esc_attr($bg_color . $bg_image . $top_pad . $right_pad . $bottom_pad . $left_pad) . '"' : '';
